...
const CalendarView: React.FC<Props> = ({ userId, onNavigate }) => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-12 relative">
      ...
      {(!activeWorkout && !activeDiet && history.length === 0) && (
        <div className="flex flex-col items-center justify-center h-[60vh] text-center">
          <Info className="w-12 h-12 text-slate-500 mb-4" />
          <h2 className="text-xl font-bold text-slate-300 mb-2">No hay datos para mostrar</h2>
          <p className="text-slate-400 mb-4">Aún no has generado una rutina ni un plan de dieta, o no hay historial disponible.</p>
          <button className="px-6 py-3 bg-brand-600 text-white rounded-xl font-bold" onClick={() => onNavigate && onNavigate('workout')}>Generar Rutina</button>
        </div>
      )}
